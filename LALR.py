...
class LALR_Parser(Parser):
    ...
    def _reduce_state(self):
        self.discarded = [False] * len(self.states)
        self.parent = [-1] * len(self.discarded)
        i = 0
        while i < len(self.states) - 1:
            ...
            while j < len(self.states):
                if self.discarded[j]:
                    j += 1
                    continue
                if self.states[i].keys() == self.states[j].keys():
                    self.discarded[j] = True
                    self.parent[j] = i
                    for lr0 in self.states[i]:
                        self.states[i][lr0] = self.states[i][lr0].union(
                            self.states[j][lr0])
                j += 1
            i += 1
        for transfer in self.dfa:
            transfer_to = self.dfa[transfer]
            if self.discarded[transfer_to]:
                self.dfa[transfer] = self.parent[transfer_to]
        for transfer in self.dfa:
            transfer_from = transfer[0]
            if self.discarded[transfer_from]:
                new_transfer = (self.parent[transfer_from], transfer[1])
                if new_transfer in self.dfa.keys():
                    if self.dfa[new_transfer] != self.dfa[transfer]:
                        raise Exception("LALR状态合并冲突")
                else:
                    self.dfa[new_transfer] = self.dfa[transfer]

    def print_states(self):
        for i in range(len(self.states)):
            print('State', i)
            if self.discarded[i]:
                print('( Discarded, annexed by state', self.parent[i], ')')
            else:
...
#     ("A", "b")
# )
parser = LALR_Parser(None)
parser.print_states()
parser.get_parsing_table('parsing_lalr.csv')
